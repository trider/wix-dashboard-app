...
import { checkout, checkoutTemplates, totalsCalculator } from "@wix/ecom";
import { httpClient } from "@wix/essentials";
import { orders } from "@wix/ecom";
import { additionalFees } from "@wix/ecom/service-plugins/context";
import { ServicePluginDefinition } from "@wix/sdk";
...
const ProductPackagingFeesPlugin: FC = () => {
  ...
  const calculateCheckoutFees = async (id: string,  payload: checkout.AddToCheckoutOptions,   options: checkout.UpdateCheckoutOptions ) => {
    const response:any = await httpClient.fetchWithAuth(`${import.meta.env.BASE_API_URL}/additional-fees/calculateAdditionalFees`,
        {
          method: "POST",
          body: JSON.stringify({
            name: "wrapping",
  ...
  const createCheckout = async (options: checkout.CreateCheckoutOptions) => {
    const response: any = await checkout
                      ...
                      checkoutInfo:{
                        shippingInfo:{
                          selectedCarrierServiceOption:{
                            code:'c4eb907-0b59-4631-baef-7c0c0bde0b59'
                          },
