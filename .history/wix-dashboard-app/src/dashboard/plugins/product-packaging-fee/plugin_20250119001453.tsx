...
const ProductPackagingFeesPlugin: FC = () => {
  const [currentProduct, setCurrentProduct] = useState<products.Product>();
    ...
    // dashboard.addSitePlugin('ef804e50-c1e3-4aeb-b935-980f2e43c31d')
    ...
    const updateCheckout = async (id:string, options:checkout.UpdateCheckoutOptions) => {
      const response = await checkout.updateCheckout(id,options).then((data)=>{
        alert(JSON.stringify(data))
        console.log(data)
        return data
      });
      const payload:checkout.CreateOrderOptions= {
        delayCapture:true
      }
    ...
    const addToCheckout = async (id:string, options:checkout.AddToCheckoutOptions) => {
      ...
      return updateCheckout(id, payload)
